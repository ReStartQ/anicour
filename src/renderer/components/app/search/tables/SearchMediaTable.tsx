...
export default function SearchMediaTable({ props }: any) {
  ...
  const { data, refetch, dataUpdatedAt }: any = useSearchQuery(
    ...
    () => [
      {
        ...
        accessorFn: (row: any) => {
          return (
            <SearchTitleTableBox title={titlePreference.title} row={row} />
          );
        },
      },
      {
  ...
  const columnsSearchManga = useMemo<MRT_ColumnDef<any>[]>(
    // column definitions...
    () => [
      {
        size: 200,
  ...
  const myTable = useMaterialReactTable({
    columns:
      myCategory.category === 0 ? columnsSearchAnime : columnsSearchManga,
